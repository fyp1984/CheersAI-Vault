...
impl NERDetector {
    ...
    pub fn new_with_ai_detection(use_ai: bool) -> Self {
        ...
        patterns.push(("手机号".to_string(), Regex::new(r"1[3-9]\d{9}").unwrap()));
        ...
        patterns.push(("护照号".to_string(), Regex::new(r"\b[EGP]\d{8}\b").unwrap()));
    ...
    pub fn detect_entities(&self, text: &str) -> Vec<EntityMatch> {
        if self.use_ai_detection {
            ...
            // 合并结果：姓名取交集，其他取并集
            let merged =
                self.merge_detections(regex_entities, ner_entities, ai_entities, search_entities);
        ...
        for (entity_type, pattern) in &self.patterns {
            for mat in pattern.find_iter(text) {
                if entity_type == "手机号" {
                    ...
                    let before_is_digit =
                        mat.start() > 0 && bytes[mat.start() - 1].is_ascii_digit();
                    let after_is_digit =
        ...
        match Self::call_ollama_with_timeout(&prompt, 15) {
            Ok(response) => {
                ...
                for line in response.lines() {
                    ...
                    // 格式2：类型：值1,值2
                    else if let Some((type_part, values_part)) =
    ...
    fn detect_names_smart(&self, text: &str) -> Vec<EntityMatch> {
        ...
        // 常见的非姓名词汇（黑名单）
        let blacklist = vec![
            "方式", "方法", "方面", "方向", "公司", "项目", "系统", "平台", "服务", "产品", "技术",
            "管理", "开发", "设计", "运营", "市场", "销售", "客户", "用户", "数据", "信息", "内容",
            "文件", "文档",
        ];
    ...
    fn has_name_context(&self, text: &str, char_pos: usize) -> bool {
        ...
        let end = if char_pos + 20 < chars.len() {
            char_pos + 20
        } else {
            chars.len()
        };
    ...
    pub fn detect_entities_batch(&self, texts: &[String]) -> Vec<Vec<EntityMatch>> {
        ...
        let rows: Vec<Vec<String>> = texts.iter().map(|text| vec![text.clone()]).collect();
