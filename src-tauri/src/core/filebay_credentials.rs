...
pub fn set_token(token: &str) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    {
        let status = std::process::Command::new("/usr/bin/security")
            ...
            .status()
            .map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_UNAVAILABLE"))?;
        if status.success() { Ok(()) } else { Err(anyhow!("FILEBAY_CREDENTIAL_STORE_WRITE_FAILED")) }
    }
...
pub fn delete_token() -> Result<()> {
    #[cfg(target_os = "macos")]
    {
        let status = std::process::Command::new("/usr/bin/security")
            ...
            .status()
            .map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_UNAVAILABLE"))?;
        if status.success() || status.code() == Some(44) { Ok(()) } else { Err(anyhow!("FILEBAY_CREDENTIAL_STORE_DELETE_FAILED")) }
    }
...
fn credential_path() -> Result<std::path::PathBuf> {
    let base = dirs_next::data_dir().ok_or_else(|| anyhow!("FILEBAY_CREDENTIAL_STORE_UNAVAILABLE"))?;
...
fn windows_protect(data: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut purpose = CRYPT_INTEGER_BLOB { cbData: WINDOWS_PURPOSE.len() as u32, pbData: WINDOWS_PURPOSE.as_ptr() as *mut u8 };
    let mut output = CRYPT_INTEGER_BLOB { cbData: 0, pbData: null_mut() };
    let ok = unsafe { CryptProtectData(&mut input, std::ptr::null(), &mut purpose, null_mut(), null_mut(), 0, &mut output) };
    if ok == 0 { return Err(anyhow!("FILEBAY_CREDENTIAL_STORE_WRITE_FAILED")); }
    let bytes = unsafe { std::slice::from_raw_parts(output.pbData, output.cbData as usize).to_vec() };
    unsafe { windows_sys::Win32::Foundation::LocalFree(output.pbData as *mut std::ffi::c_void); }
    Ok(bytes)
}
...
fn windows_unprotect(data: &[u8]) -> Result<Vec<u8>> {
    ...
    let mut purpose = CRYPT_INTEGER_BLOB { cbData: WINDOWS_PURPOSE.len() as u32, pbData: WINDOWS_PURPOSE.as_ptr() as *mut u8 };
    let mut output = CRYPT_INTEGER_BLOB { cbData: 0, pbData: null_mut() };
    let ok = unsafe { CryptUnprotectData(&mut input, std::ptr::null_mut(), &mut purpose, null_mut(), null_mut(), 0, &mut output) };
    if ok == 0 { return Err(anyhow!("FILEBAY_CREDENTIAL_STORE_READ_FAILED")); }
    let bytes = unsafe { std::slice::from_raw_parts(output.pbData, output.cbData as usize).to_vec() };
    unsafe { windows_sys::Win32::Foundation::LocalFree(output.pbData as *mut std::ffi::c_void); }
    Ok(bytes)
}
...
fn windows_get_token() -> Result<Option<String>> {
    ...
    let bytes = windows_unprotect(&std::fs::read(path).map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_READ_FAILED"))?)?;
    let token = String::from_utf8(bytes).map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_READ_FAILED"))?;
...
fn windows_set_token(token: &str) -> Result<()> {
    ...
    std::fs::write(&tmp, protected).map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_WRITE_FAILED"))?;
...
fn windows_delete_token() -> Result<()> {
    let path = credential_path()?;
    if path.exists() { std::fs::remove_file(path).map_err(|_| anyhow!("FILEBAY_CREDENTIAL_STORE_DELETE_FAILED"))?; }
    Ok(())
}
...
mod tests {
    ...
    impl MemoryCredentialStore {
        fn set(&self, token: &str) { *self.0.lock().unwrap() = Some(token.to_string()); }
        fn get(&self) -> Option<String> { self.0.lock().unwrap().clone() }
        fn delete(&self) { *self.0.lock().unwrap() = None; }
    }
