...
fn ensure_no_duplicate_sensitive_term(
    ...
    let duplicate = terms.iter().find(|existing| {
        exclude_id.is_none_or(|id| existing.id != id) && normalize_term_key(&existing.term) == term
    });

    if let Some(existing) = duplicate {
        Err(duplicate_sensitive_term_message(
            &normalize_term_key(&existing.category),
            &term,
        ))
    } else {
        Ok(())
...
/// 添加敏感词
#[tauri::command]
pub async fn add_sensitive_term(request: AddSensitiveTermRequest) -> Result<SensitiveTerm, String> {
    ...
    let existing_terms = db
        .get_sensitive_terms(None, false)
        .await
...
pub async fn add_sensitive_terms_batch(
    ...
    for req in requests {
        ...
        if !seen_keys.insert(key) {
            return Err(duplicate_sensitive_term_message(
                &request_category,
                &request_term,
            ));
        }
...
/// 删除敏感词
#[tauri::command]
pub async fn delete_sensitive_term(id: String) -> Result<(), String> {
    let db = get_database().await?;
...
/// 批量删除敏感词
#[tauri::command]
pub async fn delete_sensitive_terms_batch(ids: Vec<String>) -> Result<(), String> {
    let db = get_database().await?;
...
/// 搜索敏感词
#[tauri::command]
pub async fn search_sensitive_terms(query: String) -> Result<Vec<SensitiveTerm>, String> {
    let db = get_database().await?;
...
/// 导出敏感词为CSV
#[tauri::command]
pub async fn export_sensitive_terms_csv(output_path: String) -> Result<String, String> {
    let db = get_database().await?;
    let terms = db
        .get_sensitive_terms(None, false)
        .await
    ...
    // 写入文件
    std::fs::write(&output_path, csv).map_err(|e| format!("Failed to write CSV file: {}", e))?;
...
/// 从CSV导入敏感词
#[tauri::command]
pub async fn import_sensitive_terms_csv(file_path: String) -> Result<usize, String> {
    let db = get_database().await?;
    let existing_terms = db
        .get_sensitive_terms(None, false)
        .await
