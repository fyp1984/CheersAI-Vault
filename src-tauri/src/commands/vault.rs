                ...
                Ok(Some(config_json)) => {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&config_json) {
                        Ok(config_value) => {
                            ...
                            {
                                filebay_credentials::set_token(token).map_err(|_| {
                                    "FILEBAY_CREDENTIAL_MIGRATION_FAILED".to_string()
                                })?;
                                ...
                                if let Some(object) = safe_value.as_object_mut() {
                                    object.remove("token");
                                }
                                db.save_setting("filebay_config", &safe_value.to_string())
                                    .await
                                    .map_err(|_| "FILEBAY_CONFIG_STORAGE_FAILED".to_string())?;
                            }
...
pub async fn get_vault_db_stats() -> Result<VaultDbStats, String> {
    ...
    let last_updated: Option<(String,)> =
        sqlx::query_as("SELECT updated_at FROM filebay_configs ORDER BY updated_at DESC LIMIT 1")
            .fetch_optional(&pool)
            .await
            .map_err(|e| format!("查询失败: {}", e))?;
