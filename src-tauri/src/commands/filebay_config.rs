...
fn safe_config(legacy: &LegacyFileBayConfig) -> FileBayConfig {
    FileBayConfig {
        ...
        repo_name: legacy.repo_name.clone(),
        email: legacy.email.clone(),
        has_token: legacy
            .token
            .as_ref()
            .is_some_and(|value| !value.trim().is_empty()),
        downloaded_at: legacy.downloaded_at.clone(),
...
async fn migrate_legacy_config(path: &PathBuf) -> Result<FileBayConfig, String> {
    ...
    let legacy: LegacyFileBayConfig =
        serde_json::from_str(&content).map_err(|_| "FILEBAY_CONFIG_READ_FAILED")?;
    ...
    safe.has_token =
        filebay_credentials::has_token().map_err(|_| "FILEBAY_CREDENTIAL_STORE_UNAVAILABLE")?;
...
pub async fn delete_filebay_config(app: AppHandle) -> Result<String, String> {
    ...
    if path.exists() {
        std::fs::remove_file(path).map_err(|_| "FILEBAY_CONFIG_DELETE_FAILED")?;
    }
    let _ = filebay_credentials::delete_token();
    if let Ok(db) = Database::new().await {
        let _ = db.delete_filebay_config().await;
    }
...
async fn validate_internal(path: &PathBuf) -> Result<(LegacyFileBayConfig, FileBayConfig), String> {
    if !path.exists() || path.extension().and_then(|ext| ext.to_str()) != Some("json") {
        return Err("FILEBAY_CONFIG_INVALID".to_string());
    }
    let content = std::fs::read_to_string(path).map_err(|_| "FILEBAY_CONFIG_READ_FAILED")?;
    let legacy: LegacyFileBayConfig =
        serde_json::from_str(&content).map_err(|_| "FILEBAY_CONFIG_INVALID")?;
    if legacy.url.is_empty() || legacy.username.is_empty() || legacy.repo_name.is_empty() {
        return Err("FILEBAY_CONFIG_INVALID".to_string());
    }
...
pub async fn import_filebay_config(app: AppHandle, source_path: String) -> Result<String, String> {
    ...
    if had_token {
        save_safe_config(&source, &config)
            .map_err(|_| "FILEBAY_CREDENTIAL_MIGRATION_FAILED".to_string())?;
    }
    let db = Database::new()
        .await
        .map_err(|_| "FILEBAY_DATABASE_FAILED")?;
    db.save_filebay_config(&config.url, "", &config.username, &config.repo_name, true)
        .await
        .map_err(|_| "FILEBAY_DATABASE_FAILED")?;
