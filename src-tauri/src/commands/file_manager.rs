...
/// 获取文件列表
#[tauri::command]
pub async fn get_managed_files(page: i64, page_size: i64) -> Result<FileListResponse, String> {
    let db = Database::new().await.map_err(|e| e.to_string())?;

    let offset = (page - 1) * page_size;
    let files = db
        .get_managed_files(page_size, offset)
        .await
        .map_err(|e| e.to_string())?;
    let total = db
        .get_managed_files_count()
        .await
        .map_err(|e| e.to_string())?;
...
/// 批量删除文件
#[tauri::command]
pub async fn delete_managed_files(
    ids: Vec<String>,
...
pub async fn mark_file_uploaded(id: String, gitea_url: String) -> Result<String, String> {
    let db = Database::new().await.map_err(|e| e.to_string())?;
    db.mark_file_uploaded_to_gitea(&id, &gitea_url)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn search_managed_files(query: String, limit: i64) -> Result<Vec<ManagedFile>, String> {
    let db = Database::new().await.map_err(|e| e.to_string())?;
    db.search_managed_files(&query, limit)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_file_statistics() -> Result<serde_json::Value, String> {
    ...
    // 获取已上传到 Gitea 的文件数
    let uploaded: (i64,) =
        sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM managed_files WHERE gitea_uploaded = 1")
            .fetch_one(&db.pool)
            .await
    ...
    let total_size: (Option<i64>,) =
        sqlx::query_as::<_, (Option<i64>,)>("SELECT SUM(file_size) FROM managed_files")
            .fetch_one(&db.pool)
            .await
            .map_err(|e: sqlx::Error| e.to_string())?;
