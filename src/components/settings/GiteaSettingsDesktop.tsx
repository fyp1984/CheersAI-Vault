...
import { RefreshCw, Database, Lock, Upload, Download, FileJson, Lightbulb, AlertTriangle, CheckCircle } from 'lucide-react';

const safeError = (error: unknown, fallback: string) =>
  typeof error === 'string' && /^FILEBAY_[A-Z_]+$/.test(error) ? error : fallback;

export default function GiteaSettingsDesktop() {
  ...
  const loadStatus = async () => {
    try {
      ...
      console.error('Failed to load Gitea status');
      // 设置默认状态，避免一直加载
      setStatus({
        enabled: false,
        configured: false,
  ...
  const handleToggleEnabled = async () => {
    ...
    try {
      ...
      console.error('Failed to toggle enabled');
      setConfig({ ...config, enabled: !newEnabled });
    }
  };
          ...
          <div>
            ...
            {status?.config.has_token && !config.token && (
              <div className="mt-2 flex items-center gap-2">
                <Badge variant="success">✓ 凭据已安全保存</Badge>
