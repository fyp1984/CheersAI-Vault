...
export default function FileProcessDesktop() {
  ...
  const handleUseSandboxOutputDir = async () => {
    try {
      const sandboxPath = await tauriCommands.getSandboxDirPath();
      setOutputDir(sandboxPath);
    } catch (error) {
      console.error("Failed to get sandbox directory:", error);
      setMessage({ type: 'error', text: '获取沙箱目录失败' });
    }
  };
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button
              variant="secondary"
              size="sm"
              onClick={handleUseSandboxOutputDir}
              title="使用沙箱托管目录作为输出位置，脱敏产物可在「文件管理」中通过 FileBay 上传"
            >
              <FolderOpen className="w-4 h-4 mr-1" />
              使用沙箱目录
            </Button>
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="mt-8 p-6 bg-blue-50/60 border border-blue-100 rounded-xl">
          ...
          <ol className="space-y-2 text-sm text-blue-700">
            ...
            <li>6. 脱敏完成后，输出目录中将生成 Markdown 脱敏文件（.md）和对照映射文件（.cmap）</li>
          </ol>
        </div>
      </div>
