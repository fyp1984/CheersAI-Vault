...
/**
 * 桌面（Tauri）宿主的 `/cloud` 页面：挂载/管理内嵌子 WebView 云端工作区，
 * 注入 FileBay 自动同步脚本，失败时回退到统一访问页。逻辑与行为相对原
 * CheersAICloudBrowser.tsx 机械保持不变，仅完成文件拆分（不再需要
 * isDesktop 分支，因为本模块只在桌面宿主下被懒加载）。
 */
export default function CheersAICloudBrowserDesktop() {
        ...
        try {
          ...
          const autoSyncScript = `
            (function() {
                ...
                return originalFetch.apply(this, args).then(response => {
                  ...
                  if (isConfigApi && response.ok) {
                    ...
                    clonedResponse.json().then(data => {
                      console.log('[FileBay] received configuration metadata');
                      syncConfig(data);
                    }).catch(err => {
              ...
              // Token 只在当前 WebView 内存中短暂存在，并直接交给 Tauri 后端。
              function syncConfigWithPlainToken(config) {
                try {
                  ...
                  window.__TAURI_INTERNALS__.invoke('sync_filebay_config_from_desktop', {
                    ...
                    console.log('[FileBay] configuration synced');
                  }).catch(err => {
                    console.error('[FileBay] configuration sync failed');
                  });
